...
export function ResumeHub() {
  return (
    ...
    >
      <section className="rounded-[1.9rem] border border-white/10 bg-white/[0.04] p-5 shadow-[0_20px_60px_rgba(4,11,26,0.24)] backdrop-blur-xl sm:p-8">
        <div className="flex items-center gap-4">
          <div className="flex flex-col">
            <p className="text-[11px] font-semibold uppercase tracking-[0.32em] text-sky-100/78">
              Resume hub
            </p>
        ...
        <div className="mt-7 rounded-[1.6rem] border border-sky-200/15 bg-sky-200/6 p-4 sm:p-5">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h2 className="text-xl font-semibold text-white">{primaryResume.title}</h2>
          ...
          <div className="mt-4 flex items-center justify-between gap-3">
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="inline-flex items-center rounded-full border border-white/12 bg-white/[0.06] px-4 py-2 text-sm font-medium text-white transition hover:border-white/20 hover:bg-white/[0.1]"
            >
              View PDF
            </a>
            <a
        ...
        <div className="mt-3 space-y-3">
          {resumeCards.map((card) => (
            <article
              key={card.href}
              className="rounded-[1.35rem] border border-white/10 bg-slate-950/22 p-4"
            >
              <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <h2 className="text-lg font-semibold text-white">{card.title}</h2>
                  <p className="mt-2 text-sm leading-6 text-slate-300/85">{card.description}</p>
                </div>
                <div className="text-sm text-slate-400">
      ...
      <section className="mt-6 rounded-[1.5rem] border border-white/10 bg-white/[0.04] p-4 shadow-[0_18px_50px_rgba(4,11,26,0.16)] backdrop-blur-xl sm:p-5">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-7 text-slate-300/85">
              View the full event history in a cleaner timeline layout.
            </p>
          </div>
          <Link
            href="/career/timeline"
            className="inline-flex items-center rounded-full border border-white/10 bg-white/[0.05] px-4 py-2 text-sm font-medium text-slate-100 transition hover:border-white/20 hover:bg-white/[0.09]"
          >
            Open timeline
          </Link>
        </div>
